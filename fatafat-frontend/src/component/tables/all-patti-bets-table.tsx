...
export default function PattiBetsTable() {
  ...
  const slots = ["9.45", "11", "12.3", "2", "3.3", "5", "6.3", "8", "9.3"];
    ...
    <div className="overflow-x-auto p-10">
      <table className="min-w-full  p-10 divide-gray-200 shadow overflow-hidden sm:rounded-lg">
        <thead className="bg-gray-50">
          <tr>
            <th className="px-6 py-1 text-left text-xs font-medium bg-orange-400 text-gray-50 uppercase tracking-wider">
              Patti
            </th>
            {slots.map((slot, index) => (
              <th
                key={index}
                className="px-4 py-2 text-center text-xs bg-orange-400 font-medium text-gray-50 uppercase tracking-wider"
              >
                {formatSlot(slot)}
              </th>
            ))}
