...
export default function BetPointTable() {
  ...
  useEffect(() => {
    if (isFetched && data?.data) {
      ...
      switch (filterOption) {
        case 'today':
          filteredDataArray = data.data.filter(item => item.timestamp.startsWith(todayDateString));
          break;
        case 'yesterday':
          const yesterday = new Date(today);
          yesterday.setDate(yesterday.getDate() - 1);
          const yesterdayDateString = yesterday.toISOString().split('T')[0];
          filteredDataArray = data.data.filter(item => item.timestamp.startsWith(yesterdayDateString));
          break;
        case 'lastWeek':
          ...
          filteredDataArray = data.data.filter(item => {
            ...
            return itemDate >= lastWeekStartString && itemDate <= lastWeekEndString;
          });
          break;
        default:
          break;
  ...
  return (
    <>
      <div className="flex justify-center space-x-4 m-5">
        <button
          className={`px-4 py-2 rounded-lg ${filterOption === 'today' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          onClick={() => handleFilterOptionChange('today')}
        >
          Today
        </button>
        <button
          className={`px-4 py-2 rounded-lg ${filterOption === 'yesterday' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
          onClick={() => handleFilterOptionChange('yesterday')}
        >
          Yesterday
        </button>
        <button
          className={`px-4 py-2 rounded-lg ${filterOption === 'lastWeek' ? 'bg-blue-500 text-white' : 'bg-gray-200'}`}
