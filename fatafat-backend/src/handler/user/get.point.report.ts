    ...
    try {
      ...
      if (data) {
        const userDataMap = new Map<
          string,
          { totalBetPoint: number; totalWinBetPoint: number; timestamp: any }
        >(); // Map to store total bet points and win bet points for each user
        data.forEach((bet) => {
          const { userId, betPoint, winBetPoint, timestamp } = bet;
          const { totalBetPoint, totalWinBetPoint } = userDataMap.get(
            userId
          ) || { totalBetPoint: 0, totalWinBetPoint: 0, timestamp: 0 };
          userDataMap.set(userId, {
            totalBetPoint: (totalBetPoint || 0) + betPoint,
            totalWinBetPoint: totalWinBetPoint + (winBetPoint || 0),
            timestamp,
          });
        });
        ...
        userDataMap.forEach(
          ({ totalBetPoint, totalWinBetPoint, timestamp }, userId) => {
            userData.push({
              userId,
